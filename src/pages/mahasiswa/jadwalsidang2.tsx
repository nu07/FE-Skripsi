...
import { UsersIcon, MailIcon, DocumentTextIcon, CalendarIcon, ClockIcon, CheckCircleIcon } from "@heroicons/react/outline";
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className={`space-y-6 lg:col-span-2 {!data.tanggal_sidang ? "lg:col-span-3" : ""}`}>
            <Card>
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  ...
                  <Badge className="bg-green-400 text-green-900 border-green-200">{skripsi.status || "Status"}</Badge>
                </div>
              </CardHeader>
            ...
            {skripsi.pembimbing1 || skripsi.pembimbing2 ? (
              <Card className="shadow-sm">
                ...
                <CardContent>
                  <div
                    className={`grid gap-4 ${skripsi.pembimbing1 && skripsi.pembimbing2 ? "md:grid-cols-2" : "md:grid-cols-1 "}`}>
                    {skripsi.pembimbing1 ? (
                      <div className="p-4 border border-gray-200 rounded-lg bg-gray-50/50">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="text-gray-700">{skripsi?.pembimbing1?.nama || "Tidak ada nama"}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                    ...
                    {skripsi.pembimbing2 ? (
                      <div className="p-4 border border-gray-200 rounded-lg bg-gray-50/50">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            <UsersIcon className="h-4 w-4 text-gray-500" />
                            <span className="text-gray-700">{skripsi?.pembimbing2?.nama || "Tidak ada nama"}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <MailIcon className="h-4 w-4 text-gray-500" />
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-orange-800 leading-relaxed">
                                {data?.catatan_penguji1 || "Tidak ada catatan"}
                              </p>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-purple-800 leading-relaxed">
                                {data?.catatan_penguji2 || "Tidak ada catatan"}
                              </p>
                            </div>
                          </div>
                        </div>
              ...
              <Card className="shadow-sm">
                ...
                <CardContent className="space-y-4">
                  ...
                  {!skripsi?.response?.hasRegistered && skripsi?.response?.keduaPembimbingAcc ? (
                    <button
                      className="mt-2 bg-blue-500 text-center w-full rounded-lg border-blue-700 py-2 font-medium"
                      onClick={() => daftarSidangMahasiswa()}>
                      Daftar sidang
                    </button>
                  ) : null}
                </CardContent>
