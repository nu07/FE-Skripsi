  ...
  const getAllDosen = async () => {
    try {
      const res = await Axios.get(
        `/skripsi?page=${pagination.currentPages}&limit=${pagination.perPage}&search=${searchQuery}&showDeleted=${pagination.showDeleted}&status=${pagination.status}`
      );
  ...
  const SubmitEditData = async () => {
    try {
      ...
      await Axios.post("/set-pembimbing", {
        ...
        idPembimbing2: detailData.id_pembimbing2,
      });
      setIsEditData(false);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {dataDosen.map((person: any) => (
                    <tr key={person.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-36 truncate " title={person?.mahasiswa?.nim}>
                        {person?.mahasiswa?.nim}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-24 truncate" title={person?.judul}>
                        {person?.judul}
                      </td>
  ...
  return (
    <>
      <div className="space-y-3">
        <div>
          <label className="block text-sm font-medium text-gray-700 text-center">Bukti Pembayaran</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">ID Skripsi</label>
          <div className="mt-1">
            <input
              type="text"
              required
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Nama Pembimbing 1</label>
          <div className="mt-1">
            <select
              id="id_pembimbing1"
              name="id_pembimbing1"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Nama Pembimbing 2</label>
          <div className="mt-1">
            <select
              id="id_pembimbing2"
              name="id_pembimbing2"
