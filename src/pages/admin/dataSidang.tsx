  ...
  const getAllDataSidang = async () => {
    try {
      const res = await Axios.get(
        `/sidang?page=${pagination.currentPages}&limit=${pagination.perPage}&search=${searchQuery}&showDeleted=${pagination.showDeleted}`
      );
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {dataSidang.map((person: any) => (
                    <tr key={person.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-36 truncate " title={person.mahasiswa.nama}>
                        {person.mahasiswa.nama}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-24 truncate" title={person.mahasiswa.nim}>
                        {person.mahasiswa.nim}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-36 truncate" title={person.mahasiswa.email}>
                        {person.mahasiswa.email}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-36 truncate" title={person.tanggal_sidang}>
                        {" "}
                        {new Date(person?.tanggal_sidang).toLocaleString("id-ID", {
                          hour: "2-digit",
                          minute: "2-digit",
                          second: "2-digit",
                          day: "2-digit",
  ...
  return (
    <>
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Nama Penguji 1</label>
          <div className="mt-1">
            <select
              id="id_pembimbing1"
              name="id_pembimbing1"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Nama Penguji 2</label>
          <div className="mt-1">
            <select
              id="id_pembimbing2"
              name="id_pembimbing2"
